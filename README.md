...
[![Web Audio API](https://img.shields.io/badge/Web_Audio_API-Procedural-991b1b?style=for-the-badge)](https://developer.mozilla.org/en-US/docs/Web/API/Web_Audio_API)
...
```mermaid
flowchart TD
    Start([User Launches Application]) --> Title[Title Screen: CAPTCHA FROM HELL]
    Title -->|Click Options| OptModal[System Settings: Audio, Shake, Telemetry]
    Title -->|Click Quit| QuitModal[Surrender Dialogue: 'I Am But A Bot']
    Title -->|Click Play Gauntlet| S0[Stage 0: Checkbox Bait]

    subgraph Gauntlet [The 11-Stage Human Verification Gauntlet]
        S0 -->|Suspicious Signature Detected| S1[Stage 1: Screaming Decibel Verification\n≥ 80 dB for 3.0 continuous seconds]
        S1 -->|Acoustic Goal Met| S2[Stage 2: Affirmations Grid\nFind 1 Human Statement among 15 Decoys]
        S2 -->|Correct Tile Selected| S3[Stage 3: Ultra-Speed Whack-a-Mole\nCatch 4 Moles darting every ~0.35s]
        S3 -->|All 4 Moles Stunned| S4[Stage 4: Breakup Simulator\nMaintain firm boundaries with Rias]
        S4 -->|Clean Breakup Achieved| S5[Stage 5: Minecraft Wither Battle\nLand 40 sword clicks in 4.0 seconds]
        S5 -->|Nether Star Obtained| S6[Stage 6: Authentic Beacon Crafting\n5 Glass, 1 Star, 3 Obsidian in 3x3 Grid]
        S6 -->|Beacon Placed in Hotbar| S7[Stage 7: Happiness Exam\nWebcam Smile Dopamine Calibration]
        S7 -->|Frustration Verified| S8[Stage 8: Moving Rim Free Throws\nKinetic 2D Arc: 3 Consecutive Swishes]
        S8 -->|3 In A Row Scored| S9[Stage 9: Stock Market Valuation\nHold $ROBOT through 3 Market Dips]
        S9 -->|$1,000 Dividend Paid| S10[Stage 10: Geometry Dash Sprint\nCube -> Gravity Flip -> Mini -> Ship Flight]
    end

    S10 -->|100% Finish Line Reached| Loader[3-Second Forensic Security Dump]
    Loader --> Dossier[End Screen CRT Terminal\nHardware Diagnostics & Behavioral Roast]

    Dossier -->|Return to Title| Title
    Dossier -->|Restart Gauntlet| S0
```
